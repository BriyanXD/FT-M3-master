...
function problemA() {
  ...
  // promise version
  //
  let p1 = promisifiedReadFile("poem-two/stanza-01.txt").then((response) =>
    blue(response)
  );
  let p2 = promisifiedReadFile("poem-two/stanza-02.txt").then((response) =>
    blue(response)
  );
...
function problemB() {
  ...
  // promise version
  //
  let arrPromise = filenames.map((name) =>
    promisifiedReadFile(name).then((response) => blue(response))
  );
  Promise.all(arrPromise).then(() => console.log("done"));
}
...
function problemC() {
  ...
  // promise version
  //
  let promesas = filenames.map((name) => promisifiedReadFile(name));
